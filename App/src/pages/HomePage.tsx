...
const HomePage = () => {
  ...
  async function fetchingData() {
    try {
      ...
      const res = await fetch(url+"/api/v1/content", {
        method: "GET",
        headers: {
          "token": token
        },
  ...
  async function share() {
    try {
      ...
      if (res.ok) {
        ...
        const shareLink = `${url}/share/${userId}?data=${encodedData}`;
    ...
    <div className="bg-slate-200 w-full pb-10">
      <div className="flex justify-between">
        ...
        <div className="flex gap-2 mt-5 mr-8">
          ...
          <ButtonUi variant="secondary" size="lg" text={"Logout"} startIcon={<LogoutIcon />}
           onClick={() => {
            navigate("/"); 
           }} />
        </div>
